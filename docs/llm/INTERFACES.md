...
- Entrypoints:
  ...
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/install_simmer_ab_observe_task.ps1`
...
Weather daily mode switcher (observe-only):
- Switch weather daily execution mode (`Task Scheduler` vs `weather_daily_daemon`) consistently:
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/set_weather_daily_mode.ps1 -Mode daemon`
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/set_weather_daily_mode.ps1 -Mode task`
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/set_weather_daily_mode.ps1 -Mode off`
- Key flags:
  - `-Mode` (`daemon` / `task` / `off`)
  - `-ConfigFile`（既定 `configs/bot_supervisor.observe.json`）
  - `-DaemonJobName`（既定 `weather_daily_daemon`）
  - `-MimicTaskName`, `-Top30TaskName`（既定 `WeatherMimicPipelineDaily`, `WeatherTop30ReadinessDaily`）
  - `-NoBackground`, `-Background`

No-longshot daily mode switcher (observe-only):
- Switch no-longshot daily execution mode (`Task Scheduler` vs `no_longshot_daily_daemon`) consistently:
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/set_no_longshot_daily_mode.ps1 -Mode daemon`
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/set_no_longshot_daily_mode.ps1 -Mode task`
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/set_no_longshot_daily_mode.ps1 -Mode off`
- `-Mode task` / `-Mode off` では、repo配下で実行中の `no_longshot_daily_daemon.py`（pythonプロセス）を停止して片系運用を強制する。
- Key flags:
  - `-Mode` (`daemon` / `task` / `off`)
  - `-ConfigFile`（既定 `configs/bot_supervisor.observe.json`）
  - `-DaemonJobName`（既定 `no_longshot_daily_daemon`）
  - `-TaskName`（既定 `NoLongshotDailyReport`）
  - `-NoBackground`, `-Background`

Repo automation emergency stop (observe-only safety helper):
- Disable all Scheduled Tasks that execute under this repo and optionally stop repo-owned script processes:
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/disable_repo_tasks.ps1`
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/disable_repo_tasks.ps1 -NoBackground -NoKillProcesses`
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/disable_repo_tasks.ps1 -NoBackground -NoStopRunningTasks`
- Key flags:
  - `-RepoRoot`（既定 `C:\Repos\polymarket_mm`）
  - `-NoKillProcesses`（repo配下 `scripts\` を実行中の python/powershell 系プロセス停止をスキップ）
  - `-NoStopRunningTasks`（Disable 前の `Stop-ScheduledTask` をスキップ）
  - `-NoBackground`, `-Background`
...
- Key flags:
  ...
  - `-ShortFailureThresholdSec`, `-ShortFailureRestartDelaySec`（短時間失敗時のバックオフ。`elapsed<=threshold` なら再試行待機を長めにする。既定 `180s` / `300s`）
...
- Key flags:
  ...
  - postcheck: `-ObserveLogFile`, `-Hours`, `-ThresholdsCents`, `-MinUsefulPct`, `-MinUseful5cPct`, `-MinSamples`, `-ReportFile`, `-SummaryFile`, `-AlarmLogFile`, `-AutoRecheckOnSamplesShortfall`, `-AutoRecheckBufferMinutes`, `-AutoRecheckMinDelayMinutes`, `-AutoRecheckMessage`, `-AutoRecheckWaiterStateFile`, `-AutoRecheckLogFile`, `-AutoRecheckMarkerFile`, `-NoBackground`
  - installer: `-TaskName`, `-AlarmAt`, `-Message`, `-LogFile`, `-MarkerFile`, `-MsgTimeoutSec`, `-RunNow`, `-NoBackground`
  - action: `-Message`, `-LogFile`, `-MarkerFile`, `-MsgTimeoutSec`, `-DisableMsg`, `-PythonExe`, `-SkipStrategySnapshot`
- Behavior:
  ...
  - Runs `run_weather_24h_postcheck.ps1 -AutoRecheckOnSamplesShortfall` after alarm action to produce latest usefulness snapshot.
  - By default, also refreshes `logs/strategy_register_latest.json/html` via `python scripts/render_strategy_register_snapshot.py --pretty` (use `-SkipStrategySnapshot` to disable).
  - For sample-shortfall rechecks, set custom `-WaiterStateFile`, `-LogFile`, `-MarkerFile` to avoid overwriting the primary 24h alarm waiter/state.
  - `run_weather_24h_postcheck.ps1 -AutoRecheckOnSamplesShortfall` schedules the next gate recheck alarm when `gate_reasons` contains `samples_below_min`.
...
- Key flags:
  ...
  - `--assumed-bankroll-usd` / `-AssumedBankrollUsd` を省略した場合は、`logs/strategy_register_latest.json.bankroll_policy.initial_bankroll_usd`（fallback: `docs/llm/STRATEGY.md` の `## Bankroll Policy`）を既定 bankroll として解決する。
...
- Snapshot payload includes:
  ...
  - `kpi_core.daily_realized_pnl_usd_text`（日次実現PnLの表示値）
  ...
  - `no_longshot_status.rolling_30d_monthly_return_text`（新条件専用があれば優先表示）
  - `no_longshot_status.rolling_30d_monthly_return_source`
...
- Run one command to refresh and print concise gate status:
  ...
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check_morning_status.ps1 -FailOnSimmerAbInterimNoGo -SimmerAbInterimTarget 7d`
- Key flags:
  - `--no-refresh`（refresh一式を省略。`record_simmer_realized_daily.py` / `materialize_strategy_realized_daily.py` / `render_strategy_register_snapshot.py` / `check_strategy_gate_alarm.py` / `report_uncorrelated_portfolio.py --no-memo` / `render_implementation_ledger.py`）
  - `--skip-health`（`report_automation_health.py` の更新/判定を省略）
  - `--skip-gate-alarm`（`check_strategy_gate_alarm.py` の更新/判定を省略）
  - `--skip-uncorrelated-portfolio`（`report_uncorrelated_portfolio.py` の更新/判定を省略）
...
- Key flags:
  ...
  - installer は task action / run-now 双方で `-FailOnSimmerAbInterimNoGo` を常時付与する（設定ドリフト防止）。
  - installer の `-RunNow` は Scheduled Task の即時起動ではなく、runner を `-NoBackground` で直接1回実行する（ヘッドレス環境での `LastTaskResult=0xC000013A` 汚染回避）。
...
- Key flags:
  - `--task`（repeatable。監視対象Scheduled Task名。`?TaskName` で optional task 指定）
...
- Key flags:
  ...
  - `--no-memo`, `--pretty`
  - memo 出力時は同内容を `docs/memo_uncorrelated_portfolio_latest.txt` にも同期する。
  - output JSON `meta.strategy_scope_mode` で cohort 解釈を明示（`explicit_strategy_ids` / `adopted_from_strategy_register`）。
...
- Daily runner (PowerShell):
  ...
  - live 実行時の runner key flags: `-LiveMaxOrders`, `-LiveOrderSizeShares`, `-LiveMaxDailyNotionalUsd`, `-LiveMaxOpenPositions`, `-LiveMaxEntryNoPrice`, `-LivePriceBufferCents`, `-LiveScreenSource`（`fast` / `primary`）, `-LiveStateFile`, `-LiveExecLogFile`, `-LiveLogFile`。
  ...
  - `-RunLogPath`（既定 `logs/no_longshot_daily_run.log`）で runner 実行ログの出力先を上書き可能。既定ログが他プロセスでロック中でも別ログへ退避して実行できる。
  - runner は `logs/no_longshot_daily_run.lock` で単一起動を強制し、実行中インスタンス検知時は重複実行を避けて `exit 4` で即時終了する（stale lock は自動掃除）。
...
- No-longshot live entry helper (live optional, default observe preview):
  ...
  - key flags: `--execute`, `--confirm-live`, `--screen-csv`, `--state-file`, `--exec-log-file`, `--log-file`, `--max-new-orders`, `--order-size-shares`, `--max-daily-notional-usd`, `--max-open-positions`, `--max-entry-no-price`, `--price-buffer-cents`, `--min-liquidity`, `--min-volume-24h`, `--api-timeout-sec`, `--win-threshold`, `--lose-threshold`, `--clob-host`, `--chain-id`, `--pretty`
...
- Monthly return quick report (canonical snapshot reader):
  ...
  - `python scripts/report_no_longshot_monthly_return.py --json`
  - `python scripts/report_no_longshot_monthly_return.py --expect-source-prefix realized_rolling_30d --min-resolved-trades 10`
  - key flags: `--snapshot-json`, `--json`, `--value-only`, `--expect-source-prefix`, `--min-resolved-trades`, `--fail-on-stage-not-final`, `--out-json`
  - default output keys: `monthly_return_now_text`, `monthly_return_now_source`, `monthly_return_now_new_condition_text`, `monthly_return_now_all_text`, `rolling_30d_resolved_trades`, `realized_30d_gate_decision`
...
- Key flags:
  ...
  - Horizon/liquidity guardrails: `--min-days-to-end`, `--max-days-to-end`, `--allow-missing-end-date`, `--min-liquidity`, `--min-volume-24h`
  - Event classes now include political/macro categories (`election_politics`, `macro_policy`) in addition to merger/legal/regulatory/geopolitical families.
...
- A/B 30-day decision judge (observe-only):
  ...
  - 既定では `--min-window-hours 20` を適用し、短時間の手動比較窓（数分など）を判定集計から除外
  - 出力JSON `summary.interim_milestones` に 7日暫定 (`tentative_7d`) / 14日中間 (`intermediate_14d`) の進捗と `PENDING|GO|NO_GO` を含む
  - key flags: `--history-file`, `--min-days`, `--expectancy-ratio-threshold`, `--decision-date`, `--today`, `--min-window-hours`, `--fail-on-final-no-go`, `--output-file`, `--output-json`
- A/B daily helper: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run_simmer_ab_daily_report.ps1`
  ...
  - `logs/simmer-ab-daily-compare-history.jsonl` は `since+until` 窓キーで upsert される（同一窓の重複追記を防止）
  - run-lock: `logs/simmer-ab-daily-report.lock`（同時実行時は `exit 4` で skip）
  - key flags: `-JudgeMinDays`, `-JudgeMinWindowHours`, `-JudgeExpectancyRatioThreshold`, `-JudgeDecisionDate`, `-FailOnFinalNoGo`, `-SkipJudge`
- A/B daily task installer (observe-only):
  ...
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/install_simmer_ab_daily_task.ps1 -NoBackground -StartTime 00:05 -JudgeMinDays 25 -JudgeMinWindowHours 20 -JudgeDecisionDate 2026-03-22 -FailOnFinalNoGo -RunNow`
  - key flags: `-TaskName`, `-RepoRoot`, `-StartTime`, `-PowerShellExe`, `-PythonExe`, `-JudgeMinDays`, `-JudgeMinWindowHours`, `-JudgeExpectancyRatioThreshold`, `-JudgeDecisionDate`, `-FailOnFinalNoGo`, `-SkipJudge`, `-RunNow`
- A/B collector watchdog profile (observe-only baseline/candidate continuous capture):
  - `python scripts/bot_supervisor.py run --config configs/bot_supervisor.simmer_ab.observe.json --poll-sec 1 --write-state-sec 2 --halt-when-all-stopped`
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run_simmer_ab_observe_supervisor.ps1 -NoBackground`
  - default config uses `--daily-loss-limit-usd 0` for both baseline/candidate so collectors do not stop on intraday mark-to-market drawdown.
  - runtime files: `logs/simmer_ab_supervisor.log`, `logs/simmer_ab_supervisor_state.json`, `logs/simmer_ab_observe_supervisor.lock`
  - runner は lock file を用いて単一起動を強制する（既存インスタンス稼働時の重複起動はスキップ）。
  - worker files: `logs/simmer-ab-baseline.log`, `logs/simmer-ab-candidate.log`, `logs/simmer_ab_baseline_state.json`, `logs/simmer_ab_candidate_state.json`, `logs/simmer-ab-baseline-metrics.jsonl`, `logs/simmer-ab-candidate-metrics.jsonl`
- A/B collector watchdog task installer (observe-only):
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/install_simmer_ab_observe_task.ps1 -NoBackground`
  - `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/install_simmer_ab_observe_task.ps1 -NoBackground -StartMode Logon -RunNow`
  - key flags: `-TaskName`, `-FallbackTaskName`, `-RepoRoot`, `-PowerShellExe`, `-PythonExe`, `-ConfigFile`, `-LogFile`, `-StateFile`, `-LockFile`, `-PollSec`, `-WriteStateSec`, `-RunSeconds`, `-StartupScriptName`, `-NoStartupFolderFallback`, `-StartMode`, `-PrincipalMode`, `-RunNow`
  - task作成権限がない場合、`-FallbackTaskName`（既定 `SimmerWeatherAutoTrade`）の既存タスク再利用を試行する。
  - fallback task 再利用も失敗した場合は、既定で Startup フォルダに起動 `.cmd` を生成して次回ログオン時の自動起動を確保する（`-NoStartupFolderFallback` で無効化可能）。
...
- Dedicated A/B collector watchdog profile (observe-only, baseline+candidate):
  - `python scripts/bot_supervisor.py run --config configs/bot_supervisor.simmer_ab.observe.json --poll-sec 1 --write-state-sec 2 --halt-when-all-stopped`
  - `python scripts/bot_supervisor.py status --state-file logs/simmer_ab_supervisor_state.json`
  - `python scripts/bot_supervisor.py stop --state-file logs/simmer_ab_supervisor_state.json`
