...
def _simulate(
    ...
    quote_bid = 0.0
    quote_ask = 0.0
    quote_set_ts: Optional[dt.datetime] = None

    for s in samples:
        if quote_bid > 0 and s.best_ask_jpy <= quote_bid and (inv + order_size_btc) <= (max_inventory_btc + 1e-12):
            notional = quote_bid * order_size_btc
        ...
        if quote_ask > 0 and s.best_bid_jpy >= quote_ask and size > 1e-12:
            notional = quote_ask * size
            fee = notional * (maker_fee_bps / 10000.0)
        ...
        need_refresh = (
            quote_set_ts is None
            or quote_bid <= 0
            or quote_ask <= 0
            or (s.ts - quote_set_ts).total_seconds() >= quote_refresh_sec
        )
        if need_refresh:
            quote_bid = _q_down(s.mid_jpy - half_spread_yen, tick_size_jpy)
            quote_ask = _q_up(s.mid_jpy + half_spread_yen, tick_size_jpy)
            if quote_ask <= quote_bid:
                quote_ask = quote_bid + max(1.0, tick_size_jpy)
            quote_set_ts = s.ts
    ...
    return SimResult(
        ...
        quote_refresh_sec=quote_refresh_sec,
...
def _format_table(rows: list[SimResult]) -> str:
    header = (
        "rank  half(y)  refresh(s)  size(btc)  max_inv  fills(b/s)  pnl_total  pnl_realized  dd_max  score  turnover"
    )
    lines = [header]
...
def main() -> int:
    ...
    p.add_argument("--quote-refresh-secs", default="10,30,60,120", help="Comma-separated quote refresh seconds")
    ...
    refresh_secs = _parse_float_list(args.quote_refresh_secs)
    order_sizes = _parse_float_list(args.order_sizes_btc)
    if not half_spreads or not refresh_secs or not order_sizes:
        print("half-spreads-yen or quote-refresh-secs or order-sizes-btc is empty.")
        return 4
    ...
    print(
        "Search: "
        f"half_spreads={len(half_spreads)} refresh_secs={len(refresh_secs)} order_sizes={len(order_sizes)} "
