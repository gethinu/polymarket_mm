[CmdletBinding()]
param(
  [string]$TaskName = "FadeRegimeStagedChecks30m",
...
function Show-Usage {
  Write-Host "Usage:"
  Write-Host "  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/install_fade_regime_staged_checks_task.ps1 -NoBackground [-TaskName FadeRegimeStagedChecks30m] [-IntervalMinutes 30] [-Hours 24] [-MetricScope since_baseline] [-RunNow]"
  Write-Host "  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/install_fade_regime_staged_checks_task.ps1 -NoBackground -?"
}
...
$runnerPath = Join-Path $RepoRoot "scripts\run_fade_regime_staged_checks.ps1"
...
if ($NoControlSelfForControlArm.IsPresent) {
  $actionParts += "-NoControlSelfForControlArm"
}
if ($RefreshStrategySnapshot.IsPresent) {
  $actionParts += "-RefreshStrategySnapshot"
}
...
$desc = "Run fade regime/side staged checks periodically (observe-only)"
...
if ($RunNow.IsPresent) {
  ...
  if ($NoControlSelfForControlArm.IsPresent) {
    $runArgs += "-NoControlSelfForControlArm"
  }
  if ($RefreshStrategySnapshot.IsPresent) {
    $runArgs += "-RefreshStrategySnapshot"
  }
