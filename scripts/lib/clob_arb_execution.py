...
def trade_matches_order_ids(trade: dict, order_ids: Set[str]) -> bool:
    """True if a trade row was generated by one of our own order ids (taker or maker).
    Used so reconciliation never credits OTHER traders' fills on the same token as ours."""
    if not order_ids or not isinstance(trade, dict):
        return False
    for key in ("taker_order_id", "takerOrderId", "order_id", "orderID", "maker_order_id"):
        v = trade.get(key)
        if isinstance(v, str) and v in order_ids:
            return True
    makers = trade.get("maker_orders") or trade.get("makerOrders")
    if isinstance(makers, list):
        for m in makers:
            if isinstance(m, dict):
                for key in ("order_id", "orderID", "maker_order_id", "makerOrderId"):
                    v = m.get(key)
                    if isinstance(v, str) and v in order_ids:
                        return True
    return False
...
async def reconcile_execution(
    client, candidate: Candidate, submit_ts_ms: int, args, logger, order_ids=None
) -> dict:
    token_ids = [leg.token_id for leg, _ in candidate.leg_costs]
    target = candidate.shares_per_leg * args.min_fill_ratio
    own_ids: Set[str] = {str(o) for o in (order_ids or []) if isinstance(o, str) and o.strip()}
    attribute_by_id = bool(own_ids)
    if not attribute_by_id:
        logger.info(
            f"[{iso_now()}] live: WARNING reconcile cannot attribute fills by order id; "
            f"falling back to asset+time window (may count foreign trades)"
        )
    ...
    for _ in range(args.reconcile_polls):
        ...
        for tid in token_ids:
            ...
            for tr in rows:
                # Only count trades attributable to our own orders. The asset+time
                # window alone would credit unrelated traders' fills as ours and
                # produce a false all_filled (=> naked, unhedged legs).
                if attribute_by_id and not trade_matches_order_ids(tr, own_ids):
                    continue
                trid = tr.get("id")
...
async def execute_with_retries_clob(client, candidate: Candidate, books: Dict[str, LocalBook], args, logger) -> Tuple[bool, dict]:
    ...
    for attempt in range(1, args.exec_max_attempts + 1):
        ...
        recon = await reconcile_execution(client, candidate, submit_ts_ms, args, logger, order_ids=order_ids)
...
async def maybe_execute_candidate_live(
    *,
    ...
    if ok:
        ...
        if exec_backend == "clob":
            # Daily notional = capital deployed (basket_cost), consistent with the
            # gate's reservation in can_execute_candidate. Do NOT fold slippage/cost
            # estimate into the notional accounting (that made the gate and the
            # accrual use different bases, letting cumulative notional drift past cap).
            state.notional_today += notional_inc  # notional_inc == candidate.basket_cost
            logger.info(
