...
def test_mode_daemon_keeps_matching_daemon_process_running_and_enables_config(tmp_path: Path):
    repo_root = tmp_path / "repo"
    cfg_dir = repo_root / "configs"
    cfg_dir.mkdir(parents=True, exist_ok=True)
    cfg_path = cfg_dir / "bot_supervisor.observe.json"
    cfg_path.write_text(
        json.dumps(
            {
                "jobs": [
                    {"name": "no_longshot_daily_daemon", "enabled": False},
                ]
            },
            ensure_ascii=True,
        ),
    ...
    try:
        ...
        res = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
        assert res.returncode == 0, f"stdout={res.stdout}\nstderr={res.stderr}"
        assert daemon_proc.poll() is None, "daemon process should stay running in daemon mode"
        out = (res.stdout or "") + (res.stderr or "")
        assert "daemon_processes_stopped=0" in out

        cfg = json.loads(cfg_path.read_text(encoding="utf-8"))
        job = next(j for j in cfg.get("jobs", []) if str(j.get("name")) == "no_longshot_daily_daemon")
        assert bool(job.get("enabled")) is True
    finally:
        if daemon_proc.poll() is None:
            daemon_proc.kill()
